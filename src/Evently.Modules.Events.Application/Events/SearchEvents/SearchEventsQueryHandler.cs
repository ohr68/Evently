...
{
    public async Task<Result<SearchEventsResponse>> Handle(SearchEventsQuery request,
        ...
        const string sql =
            $"""
             ...
             WHERE 
                 ...
                 (@StartDate IS NULL OR starts_at_utc >= @StartDate::timestamp) AND 
                 (@EndDate IS NULL OR ends_at_utc <= @EndDate::timestamp)
